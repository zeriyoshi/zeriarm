...
if [ ! -d "${TARGET_MOUNT_POINT}" ]; then
    "${TWEET_BIN}" "[image_backup] ${TARGET_MOUNT_POINT} missing. ${TWEET_AT}"
fi

if [ ! -d "${TARGET_MOUNT_POINT}${TARGET_DIR}" ]; then
    "${TWEET_BIN}" "[image_backup] ${TARGET_MOUNT_POINT}${TARGET_DIR} missing. ${TWEET_AT}"
fi

OUT_PATH="${TARGET_MOUNT_POINT}${TARGET_DIR}/$(date +"%Y%m%d").img.gz"
"${TWEET_BIN}" "[image_backup] image backup started: ${OUT_PATH} ${TWEET_AT}"
dd if="${TARGET_DEV}" bs=1M | gzip -c > "${OUT_PATH}"
"${TWEET_BIN}" "[image_backup] image backup finished: ${OUT_PATH} ${TWEET_AT}"
